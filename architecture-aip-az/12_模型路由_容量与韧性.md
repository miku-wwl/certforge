...
flowchart TD
    ...
    R -->|Routing mode: Cost / Balanced / Quality| MODE[Router routing mode]
    MODE --> POOL[Router Eligible Model Pool]
    POOL --> POST[Post-inference Evaluation / Confidence]
    APIM -->|固定模型、固定吞吐| PT[Direct Provisioned Deployment]
    APIM -->|严格区域/参数控制| DZ[Direct Regional / Data Zone Deployment]
    APIM -->|异步/重型工作负载| Q[Service Bus]
    Q --> J[Container Apps Job / Large Model]
    PT --> POST
    DZ --> POST
    J --> POST
    POST -->|低质量/低置信度| REM[异步 remediation]
    POST -->|通过| RESP[返回响应]
```
...
| 信号 | Azure 方案 |
|---|---|
| 按请求复杂度优化成本/质量 | Foundry Model Router 的 routing mode：Cost、Balanced、Quality；它在自己的 eligible model pool 内选择模型；当前 Router 部署类型为 Global Standard 或 Data Zone Standard |
...
- Router 在自己的 deployment 内按 routing mode 选择 eligible model pool；Cost、Balanced、Quality 是路由模式，不是模型池，也不是三个下游部署。除文档明确要求的 Claude 等模型外，不需要把底层候选模型逐个部署。
- Provisioned、严格单区域或固定参数模型是 APIM/应用选择的**直接部署旁路**，不是 Router 会路由到的三个“容量分支”。
- Low confidence/low quality 是推理完成后才能获得的结果信号；只有存在前置分类器时，才能把 confidence 作为 pre-routing 条件。默认应将它接到 post-inference evaluation，再进入异步 remediation。
- 响应会披露实际选择的模型，应写入遥测；
